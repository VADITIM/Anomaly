...
- **Stat blueprints**: `EntityStats` (`#Scripts/Entities/EntityStats.cs`) is the designer-authored blueprint of all shared entity stats (MaxHealth, Speed, Damage, Armor, Weight, Tenacity/MaxTenacity, MaxStaggers, CameraPriority, AttackDuration, knockback/tenacity flags), applied in `Entity.ApplyEntityStats`. `EnemyStats : EntityStats` adds enemy-only stats (CorruptionReward, SoulReward, DamageType, WeaknessType, ChaseRange, AttackRange), applied in `Enemy.ApplyEnemyStats` when the assigned blueprint is an `EnemyStats`. `.tres` blueprints live in `#Scenes/Resources/EntityStats/`. Migration in progress: `Bee.tres`/`PracticeDummy.tres` exist but are not yet assigned in their scenes, so the subclass constructors remain the live authority until an editor assignment overrides them (then the constructor values are deleted).
...
- Player resource bars: `PlayerResourceBars` (formerly `SegmentManager`) drives the `TextureProgressBar` meters from `PlayerStats`. It resolves the six bar nodes by name (`GetNode`, not exported refs), caches their authored base **widths** one frame after `_Ready` (layout is unresolved in `_Ready`, so sizes read zero), and never writes a bar's height. Bars no longer draw segments: each stat's `UpgradeLevels` widens its bar by `WidthPerUpgrade` (5px) to the right — Health drives the Health bar (Health Star follows the right edge) and the Vessel bar's width (its max value stays 100); Stamina drives the Stamina bar/star.
...
- Death: enemy death (weapon or not) routes through `StateMachine.RequestDeath` → `OnDied` → `GrantDeathRewards` (rewards skip silently without a Player). Each enemy grants `CorruptionReward` (per-enemy `[Export]`, 1–10) to the player's Corruption meter plus `SoulReward`; Vessel is **not** granted on kill — it fills per hit via `ResourceManager.AddVesselCharge`. Player death waits 2.5 s then `SaveManager.ReloadFromDisk()` — interim loop until the §3.13 SavePoint location map exists.
- Damage flash: `CommonDamageFlash` (`IEntityBehavior`) flashes only the entity's `Sprite` node. Because a material authored in a scene is a shared sub-resource across every instance, it duplicates the material per instance in `OnReady` — otherwise all instances of an enemy type flash together.
- Weapon Arc authority: `WeaponStats` defaults are the Scythe baseline (Damage 5, StaminaCost 10, TenacityDamage 10); Arc flavor applies through `WeaponArc` multipliers (`Damage/Tenacity/Penetration/StaminaCost`) at point of use and is never written back into `WeaponStats`.
