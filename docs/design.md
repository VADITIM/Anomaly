...
The project follows **standard .NET naming conventions**.

| Category                                                            | Casing                          |
| ------------------------------------------------------------------- | ------------------------------- |
| Classes, structs, enums, methods, properties, events, public fields | PascalCase                      |
| Interfaces                                                           | `I` prefix (`IEntityBehavior`)  |
| Private and protected instance fields                                | `_camelCase`                    |
| Local variables and parameters                                       | camelCase                       |
| Constants (`const`, `static readonly`)                               | PascalCase                      |
| `[Export]` members                                                   | PascalCase properties           |

Do not simplify names. Always name each category fully — e.g. `StateMachine` instead of `SM` or similar abbreviations.

Avoid mixing naming styles within the same class. Legacy code using bare-camelCase private fields or UPPER_SNAKE constants is migrated opportunistically: only in files already being edited, whole class at a time.
...
// Wrong
private float moveSpeed;                    // missing underscore prefix
private const float HEAL_DURATION = 1.5f;  // UPPER_SNAKE is not C# style
