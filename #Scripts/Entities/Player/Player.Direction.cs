...
public partial class Player
{
    private string GetDirectionFromAngle(float angleDegrees, out bool flipH)
    {
        ...
        if (angleDegrees >= 22.5f  && angleDegrees < 67.5f)   return "SE";
        if (angleDegrees >= 67.5f  && angleDegrees < 112.5f)  return "S";
        if (angleDegrees >= 112.5f && angleDegrees < 157.5f)  return "SW";
        if (angleDegrees >= 157.5f || angleDegrees < -157.5f) return "W";
        if (angleDegrees >= -157.5f && angleDegrees < -112.5f) return "NW";
        if (angleDegrees >= -112.5f && angleDegrees < -67.5f)  return "N";
